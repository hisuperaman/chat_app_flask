...
def login():
    if app_admins.query.filter_by(uid=1111).first()==None:
        ...
        db.session.commit()
        onlyadmin = app_admins(uid=1111, firstname='John', lastname='Doe', username='admin', password=admin_password)
        db.session.add(onlyadmin)
        db.session.commit()

        logininfo.query.delete()
        db.session.commit()
        admin = logininfo(uid=1111, firstname='John', lastname='Doe', username='admin', password=admin_password)
